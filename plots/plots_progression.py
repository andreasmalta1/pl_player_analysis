...
def plt_progression_90s(df, team_name, fotmob_id, lge, comp):
    ...
    for index, row in df.iterrows():
        if row["PrgP90"] > passes_limit or row["PrgC90"] > carries_limit:
            ax.annotate(row["Player"], (row["PrgP90"] + 0.1, row["PrgC90"]))
...
def plt_progression_combined_90s(df, lge, comp):
    comp_description = "All Comps"
    if comp != "comps":
        comp_description = LEAGUES.get(comp).get("lge_name")
    lge_id = LEAGUES.get(lge).get("fotmob_id")
    ...
    save_figure(
        f"figures/{lge}/progression/{comp}/all_progression_90s.png",
        300,
        False,
        "#EFE9E6",
        "tight",
...
def plt_progression_combined_all_90s(df, comp):
    if comp == "comps":
        comp_description = "All Competitons"
    else:
        comp_description = "League Games"
    ...
    for index, row in df.iterrows():
        if row["PrgP90"] > passes_limit or row["PrgC90"] > carries_limit:
            ax.annotate(row["Player"], (row["PrgP90"] + 0.2, row["PrgC90"] + 1))

    annotate_axis(ax)

    save_figure(
        f"figures/combined/progression/all_progression_{comp}_90s.png",
        300,
        False,
        "#EFE9E6",
        "tight",
